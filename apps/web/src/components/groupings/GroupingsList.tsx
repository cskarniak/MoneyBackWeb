...
export function GroupingsList() {
  ...
  const { data, isLoading, error } = useRegroupements({ page, limit, search, sortBy, sortOrder });
  ...
  const columns = useMemo<ColumnDef<Regroupement>[]>(
    () => [
      {
        id: 'cursor',
    ...
    <Box style={{ maxWidth: 'var(--crud-list-max-width)', margin: '0 auto' }}>
      <Stack gap={0}>
        ...
        {error && (
          ...
          >
            <Text size="sm">Erreur lors du chargement des regroupements.</Text>
          </Alert>
        )}
        ...
        >
          ...
          >
              {data ? `${data.total} regroupement${data.total !== 1 ? 's' : ''}` : '…'}
          </Text>
