...
export function CategoriesList() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      notifications.show({ message: `${count} catégorie(s) exportée(s)`, color: 'green' });
    } catch {
      notifications.show({ message: "Impossible d'exporter les catégories.", color: 'red' });
    } finally {
  ...
  const columns = useMemo<ColumnDef<Category>[]>(
    () => [
      ...
      {
        id: 'actions',
        header: () => <span style={{ ...thStyle(), borderRight: 'none' }}>Actions</span>,
        cell: ({ row }) => (
      ...
      <Stack gap={0}>
        ...
        >
          <Group justify="space-between" align="center" wrap="nowrap">
            <Text inherit fw={700}>Liste des catégories</Text>
        ...
        {error && (
          ...
          >
            <Text size="sm">Erreur lors du chargement des catégories.</Text>
          </Alert>
        )}
                    ...
                    >
                      {row.getVisibleCells().map(cell => (
                        <Table.Td
                          key={cell.id}
                          style={{
                            ...tdStyle(cell.column.id !== 'actions'),
        ...
        >
          ...
          >
              {data ? `${data.total} catégorie${data.total !== 1 ? 's' : ''}` : '…'}
          </Text>
