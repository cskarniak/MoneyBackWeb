...
export function MovementTypesList() {
  ...
  const { data, isLoading, error } = useMovementTypes({ page, limit, search, sortBy, sortOrder });
  ...
  const handleExport = async () => {
    ...
    try {
      const count = await exportPaginatedListToExcel({
        ...
        filenameBase: 'types-mouvement',
      });
      notifications.show({ message: `${count} type(s) de mouvement exporté(s)`, color: 'green' });
    } catch {
      notifications.show({ message: "Impossible d'exporter les types de mouvement.", color: 'red' });
    } finally {
  ...
  const columns = useMemo<ColumnDef<MovementType>[]>(
    () => [
      {
        id: 'cursor',
        ...
        cell: ({ row }) => (
          <Group gap={4} wrap="nowrap">
            <ActionIcon
              variant="subtle"
              size="md"
              onClick={() => router.push(`/referentiels/types-mouvement/${row.original.id}`)}
              title="Modifier"
      ...
      <Stack gap={0}>
          ...
          <Group justify="space-between" wrap="nowrap">
            <Group gap={8} wrap="nowrap">
              ...
              <Button
                ...
                onClick={() => router.push('/referentiels/types-mouvement/new')}
        ...
        {error && (
          <Alert color="red" style={{ background: '#fff5f5', border: '1px solid #ffc9c9', borderTop: 'none', borderRadius: 0, padding: '10px 14px' }}>
            <Text size="sm">Erreur lors du chargement des types de mouvement.</Text>
          </Alert>
        )}
        ...
        >
          ...
          >
            {data ? `${data.total} type${data.total !== 1 ? 's' : ''} de mouvement` : '…'}
          </Text>
