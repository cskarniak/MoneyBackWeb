...
function toPayload(values: FormValues): MovementTypePayload {
  return {
    label: values.label,
...
export function MovementTypesFiche({ id }: Props) {
  const router = useRouter();
  const isNew = !id;

  const { data: movementType, isLoading: loadingMovementType } = useMovementType(id ?? '');
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (isNew) {
        ...
        router.push(`/referentiels/types-mouvement?highlight=${created.id}`);
      } else {
        await updateMutation.mutateAsync({ id: id!, ...payload });
        router.push(`/referentiels/types-mouvement?highlight=${id}`);
      }
    } catch (err: unknown) {
      void err;
    }
