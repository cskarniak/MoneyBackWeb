...
export function NotesList() {
  ...
  const sortBy = (searchParams.get('sortBy') as 'title' | 'createdAt' | 'updatedAt') ?? 'updatedAt';
  ...
  const handleSort = (col: 'title' | 'createdAt' | 'updatedAt') => {
  ...
  const handleDelete = async (note: Note) => {
    ...
    try {
      ...
      setDeleteError(`Impossible de supprimer "${note.title}".`);
    }
  };

  const sortIcon = (col: string) => (sortBy === col ? (sortOrder === 'asc' ? ' ▲' : ' ▼') : '');

  const thStyle = (col?: 'title' | 'createdAt' | 'updatedAt') => ({
  ...
  const columns = useMemo<ColumnDef<Note>[]>(
    () => [
      ...
      },
      {
        accessorKey: 'title',
        header: () => (
          <span style={thStyle('title')} onClick={() => handleSort('title')}>
        ...
        cell: ({ row }) => (
          <Group gap={4} wrap="nowrap">
            <ActionIcon
              variant="subtle"
              size="md"
              onClick={() => router.push(`/notes/${row.original.id}?${searchParams.toString()}`)}
              title="Modifier"
      ...
      <Stack gap={0}>
        ...
        >
          <Group justify="space-between" align="center" wrap="nowrap">
            <Text inherit fw={700}>Liste des notes</Text>
          ...
          <Group justify="space-between" wrap="nowrap">
            <Group gap={8} wrap="nowrap">
              ...
              <Button
                ...
                onClick={() => router.push(`/notes/new?${searchParams.toString()}`)}
        ...
        {error && (
          <Alert color="red" style={{ background: '#fff5f5', border: '1px solid #ffc9c9', borderTop: 'none', borderRadius: 0, padding: '10px 14px' }}>
            <Text size="sm">Erreur lors du chargement des notes.</Text>
          </Alert>
        )}
                    ...
                    >
                      {row.getVisibleCells().map(cell => (
                        <Table.Td
                          ...
                          onClick={cell.column.id === 'actions' ? undefined : () => router.push(`/notes/${row.original.id}?${searchParams.toString()}`)}
                        >
