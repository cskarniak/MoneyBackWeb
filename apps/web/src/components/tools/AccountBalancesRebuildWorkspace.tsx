...
import { useRebuildAccountBalances } from '@/hooks/useAccounts';
...
export function AccountBalancesRebuildWorkspace() {
  const rebuildMutation = useRebuildAccountBalances();
  ...
  return (
    <Box style={{ padding: '20px 24px' }}>
      <Stack gap={18} style={{ maxWidth: 900, margin: '0 auto' }}>
        <Text fw={700} fz={22}>Recalcul des soldes comptes</Text>
        ...
        >
          ...
          <Stack gap={16} style={{ padding: '18px 20px' }}>
            ...
            <Text fz={13} c={TEXT_MUTED}>
              Rejoue toutes les opérations non clôturées jusqu&apos;à la date de référence choisie pour
              recalculer le solde de chaque compte (solde de clôture ou d&apos;ouverture + opérations),
              puis l&apos;enregistre sur le compte avec cette date de référence. Un compte fermé est
              toujours stocké à 0.
            </Text>
        ...
        >
          ...
          {!result ? (
            ...
            <Group style={{ padding: '18px 20px' }}>
              <Text fw={600}>{result.updatedCount} compte(s) mis à jour</Text>
