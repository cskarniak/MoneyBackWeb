...
export function AccountsList() {
  ...
  const { data, isLoading, error } = useAccounts({ page, limit, search, sortBy, sortOrder });
  ...
  const handleSort = (col: 'name' | 'agency' | 'number') => {
  ...
  const thStyle = (col?: 'name' | 'agency' | 'number') => ({
  ...
  const columns = useMemo<ColumnDef<Account>[]>(
    () => [
      ...
      },
      {
        accessorKey: 'agency',
        header: () => (
          <span style={thStyle('agency')} onClick={() => handleSort('agency')}>
      ...
      },
      {
        accessorKey: 'number',
        header: () => (
          <span style={thStyle('number')} onClick={() => handleSort('number')}>
        ...
        cell: ({ row }) => (
          <Group gap={4} wrap="nowrap">
            <ActionIcon
              variant="subtle"
              size="md"
              onClick={() => router.push(`/comptes/${row.original.id}`)}
              title="Modifier"
      ...
      <Stack gap={0}>
          ...
          <Group justify="space-between" wrap="nowrap">
            <Group gap={8} wrap="nowrap">
              ...
              <Button
                ...
                onClick={() => router.push('/comptes/new')}
        ...
        {error && (
          <Alert color="red" style={{ background: '#fff5f5', border: '1px solid #ffc9c9', borderTop: 'none', borderRadius: 0, padding: '10px 14px' }}>
            <Text size="sm">Erreur lors du chargement des comptes.</Text>
          </Alert>
        )}
